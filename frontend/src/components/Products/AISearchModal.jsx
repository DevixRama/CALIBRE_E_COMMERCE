...
const AISearchModal = () => {
  ...
  const exampleText = [
    'Air conditioner',
    'find sports item for men',
    'find table for home'
  ]

  const dispatch = useDispatch();
  const handleSearchWithAI = (e) => {
    e.preventDefault()
  ...
  <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div className="bg-background/95 backdrop-blur-md border border-border rounded-2xl p-8 w-full max-w-2xl">
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => dispatch(toggleAIModal())} className="p-2 rounded-lg hover:bg-secondary transition-colors"><X className="w-5 h-5 text-foreground" /></button>
      </div>
      ...
      <div className="mt-6">
        ...
        <div className="flex flex-wrap gap-2">
          {exampleText.map((example) => (
            <button key={example} onClick={() => setUserPrompt(example)} className="px-3 py-1 bg-secondary text-foreground rounded-full text-sm hover:bg-primary hover:text-primary-foreground transition-colors">{example}</button>
          ))}
        </div>
      </div>
    </div>
