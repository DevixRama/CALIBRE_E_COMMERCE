...
import { Search, Sparkles, Star, Filter, Package } from "lucide-react";
...
const Products = () => {
  ...
  return (
    <>
      <div className="flex w-full mx-auto max-w-7xl h-full max-h-[80vh] flex-col md:flex-row gap-6">
        <aside className={`w-full md:w-1/4 p-4 rounded-2xl border border-gray-200 bg-white h-full ${isMobileFilterOpen ? "block" : "hidden md:block"}`}>
        ...
        <div className="mx-auto w-full bg-red-900 h-full flex-col">
          ...
          <main className="flex justify-center overflow-y-scroll mb-3 max-h-[68vh]">
            {products?.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div>
              : (<p className="p-6 text-gray-400 text-md">No product found.</p>)
                }
          </main>
