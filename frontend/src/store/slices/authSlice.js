...
export const forgetPassword = createAsyncThunk(
  "auth/forget/password", async (email, thunkAPI) => {
    try {
      const res = await axiosInstance.post(`/auth/password/forgot?frontendUrl=${import.meta.env.VITE_FRONTEND_URL}`, email)
      toast.success(res.data.message)
      return null
    } catch (error) {
      toast.error(error.response.data.message)
...
export const resetPassword = createAsyncThunk(
  "auth/reset/password", async ({ resetToken, password, confirmPassword }, thunkAPI) => {
    try {
      const res = await axiosInstance.put(`/auth/password/reset/${resetToken}`, { password, confirmPassword })
      toast.success(res.data.message)
      return res.data.user
    } catch (error) {
